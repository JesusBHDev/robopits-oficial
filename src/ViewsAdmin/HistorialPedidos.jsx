                        ...
                        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {historial.map((pedido) => (
                                <li key={pedido._id} className="bg-white p-4 rounded-md shadow-md">
                                    <p><strong>Cliente:</strong> {pedido.informacion.cliente.nombre}</p>
                                    <p><strong>Estado:</strong> {pedido.informacion.estado}</p>
                                    <p><strong>Punto de Retiro:</strong> {pedido.informacion.puntoDeRetiro}</p>
                                    <p><strong>Fecha del pedido:</strong> {moment(pedido.informacion.createdAt).format('DD/MM/YYYY HH:mm')}</p>
                ...
                {selectedPedido && (
                    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-blue-800 bg-opacity-50">
                        <div className="bg-white p-8 rounded shadow w-11/12 h-5/6 overflow-y-auto">
                            ...
                            <p><strong>Pedido ID:</strong> {selectedPedido._id}</p>
                            <p><strong>Fecha del pedido:</strong> {moment(selectedPedido.informacion.createdAt).format('DD/MM/YYYY HH:mm')}</p>
                            <p><strong>Cliente:</strong> {selectedPedido.informacion.cliente.nombre}</p>
                            <p><strong>Total:</strong> ${selectedPedido.informacion.total}</p>
                            <p><strong>Total de productos:</strong> {selectedPedido.informacion.totalproductos}</p>
                            <p><strong>Estado:</strong> {selectedPedido.informacion.estado}</p>
                            <p><strong>Dirección:</strong> {selectedPedido.informacion.direccion}</p>
                            <p><strong>Punto de Retiro:</strong> {selectedPedido.informacion.puntoDeRetiro}</p>
